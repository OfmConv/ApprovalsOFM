...
// import { getProfile, requestChange } from "@/services/api";
...
//         setChanges((prev: any) => ({ ...prev, [key]: val })); // <-- catat perubahan
//     };
...
//                 </div>
//                 : isChange === 1 ?
//                     <EducationSection nkp={nkp}/>
//                     : isChange === 2 ?
//                            <ReligiousFeastSection nkp={nkp} />
//                         : isChange === 3 ?
//                              <CommunitySection nkp={nkp} />
//                             : isChange === 4 ?
//                                 <PhotoGallerySection />
//                                 : <>
...
export default function ProfilePage() {
    ...
    // true  -> yang login admin, edit langsung ke tabel utama (tanpa approval)
    // false -> yang login anggota, edit lewat alur request-change + approval
    ...
    const handleSubmit = async () => {
        // Tidak ada perubahan, tidak perlu request/patch
        ...
        try {
            if (isAdmin) {
                // Admin: langsung update ke tabel utama, tanpa approval
                await updateProfileDirect(nkp, changes);
            } else {
                // Anggota: lewat alur request-change (masuk tabel temp + approvals)
                await requestChange(nkp, "Update data pribadi", changes);
            }
            setChanges({});
            setSuccessModalOpen(true);
