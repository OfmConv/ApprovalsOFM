...
export default function EducationSection({ nkp, isAdmin = false }: { nkp: string; isAdmin?: boolean }) {
    ...
    const handleSaveForm = async () => {
        ...
        try {
            ...
            if (isAdmin) {
                await updateEducationDirect(nkp, action, payload, editingRecord?.education_id);
                setFormOpen(false);
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (isAdmin) {
                await updateEducationDirect(nkp, "delete", payload, deleteTarget.education_id);
                setSuccessChanges(payload);
                setSuccessModalOpen(true);
